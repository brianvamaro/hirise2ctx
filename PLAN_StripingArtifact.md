...
| **H4 🟡 PILOT DONE 2026-07-09b — PASS (mechanically), trend-guard caveat** | **overlap-constrained leveling of per-frame predictions** (F-mode only; ≠ D, ≠ E) | whatever survives H1–H3 | per-frame additive offset in logit domain, least-squares over **co-located prediction disagreements in frame-overlap regions** (same ground, two frames → the disagreement is artifact BY CONSTRUCTION; no geology assumption, so it sidesteps D's circularity) + smooth regional-trend guard; solved on the seam/overlap graph, applied post-hoc. **⚠ scope limit:** needs per-frame predictions with overlaps (the F deployment). Boundary-discontinuity-only leveling on the mosaic map (a partition, no overlaps) is **option D — RULED OUT (circular), stays ruled out**. Frames with no overlapping partner get interpolated offsets from the graph (flagged in H6). **Motivating case: F02** (post-H1 residual block) is −2.23σ darker than incidence predicts, 2014 vs 2008 epoch, over-predicts P(rich) 0.222 vs ≤0.07 on the same ground it overlaps → a pure radiometric offset (DECISIONS 2026-07-07, `_f02_diagnose.py`). | days | blocks visually gone; validation-leg ρ not degraded |
| | *result* | | **PASS on the pilot (first PHASE-2 lever to move η² to the bar WITHOUT collapsing skill).** 7-frame E8_N44 solve (15 overlap edges), λ picked by leave-one-edge-out CV (λ*=300; near-flat across λ). Full offsets: **partition η² 0.128→0.0505** (crosses the ≲0.05 bar; λ=1000→0.0466), median 0.081→0.052, and the decisive non-circular gate — **held-out edge-CV \|Δp\| 0.074→0.035 (halved, FLAT across λ ⇒ offsets generalize, not memorize)**. Skill preserved *by construction* (additive logit offset can't change within-frame ranking — H3's collapse mechanism is structurally impossible; pooled-metric leg-B confirmation still pending). Offset signs sane: F02 gets the most negative offset (−1.54, pushes the dark over-predictor down). **⚠ Caveat:** trend guard fires — 58% of offset variance is a smooth lon/lat plane; residual-only (conservative) leaves partition η² 0.0595 and barely moves \|Δp\| (0.074→0.070). But on 7 frames the plane is underpowered and demonstrably MIS-attributes F02's known per-frame 2014 radiometric offset to a spatial trend, so residual-only understates H4; clean artifact-vs-regional-gradient separation needs the dense 907-frame graph. `scripts/f_h4_level.py`; `f_h4_leveling_{summary.csv,choropleth.png}`, `f_h4_offsets.csv`, `f_h4_offset_scatter.png`. DECISIONS 2026-07-09b(H4). | | leg-B skill instruments (§3.1) next → then reopening call |
| | *leg-B skill* | | **PASS — leveling preserves skill on real LOIO predictions** (`scripts/f_h4_legb.py`; DECISIONS 2026-07-09b(H4-legB); notebook 28 §9). Per-frame offsets solved on the 28-training-obs graph, applied at obs level to the composite LOIO preds: **Δ pooled PR-AUC (H1+H4 − H1) −0.0104** (within −0.02 gate), still **+0.019 above mosaic**, **Δ per-image AUC exactly 0** (structural claim confirmed on real data). prec@5% 0.972→0.968. ⚠ Graph fragmented at this scale (58 frames / 47 edges / **21 components** ⇒ mostly within-obs leveling); obs-level application exact for single-frame obs, mean-of-frames approx for composites (deploy-faithful per-frame LOIO = build-scale, deferred). | | THEMIS-ρ leg + trend-guard (build-time) → reopening call |
...
both gates); H4 PILOT DONE (2026-07-09b, PASS mechanically — full offsets take partition η²
0.128→0.0505 and HALVE the held-out edge-CV disagreement 0.074→0.035 with skill preserved by
construction; trend-guard caveat below).** H1 remains the operating baseline; H1+H4 is the first
stack to reach the reopening bar. **H2 + H3 together show the per-frame block variance in
predictions is not separable from geology by any data-driven *invariance* instrument** — but H4
(a *post-hoc leveling* instrument, orthogonal axis) succeeds where they failed: it removes the
F02-class level term WITHOUT touching within-frame ranking, so it cannot collapse dynamic range the
way H3's training penalty does.

**⚠ Open on H4:** the pilot's trend guard fires (58% of offset variance is a smooth spatial plane),
and on only 7 frames the plane cannot cleanly separate a genuine regional gradient from a smoothly-
varying artifact (residual-only, treating the plane as geology, leaves η² 0.0595). That separation
resolves only on the dense 907-frame graph. **Brian ruled (2026-07-09b): trust the full offsets (the
7-frame guard is underpowered), and the smooth/artifact separation is a build-time item.** **Leg-B
skill instrument DONE (2026-07-09b) — PASS:** applying the offsets costs Δ pooled PR-AUC −0.0104
(within gate), keeps H1+H4 +0.019 above the mosaic, and moves per-image AUC by exactly 0 — skill
