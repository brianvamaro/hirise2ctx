...
- **Status:** OPEN · **Severity:** medium · **Liveness:** dead-closed for the shipped map; live for every GBM/W1 number off `dataset_v2/features/` · **Verified:** no
...
- **Status:** OPEN · **Severity:** medium · **Liveness:** dead-closed for the shipped map; live for the GBM matrix and the W1 error atlas · **Verified:** no
...
- **Status:** **FIXED 2026-08-05.** All six redirected; **the full suite is now non-mutating**, verified by an identical mtime checksum over `dataset*/` + `cache*/` before and after a complete `pytest` run (511 passed). · **Severity:** high · **Liveness:** live-shipped · **Verified:** **yes — reproduced by accident, which is how it was found**
...
- **Fix as applied:** the three `dataset/`-writing tests take `tmp_path` (the features one stages its labels into the tmp tree, since Stage 4b reads labels from `output_dir`). The three `cache/`-writing tests use a new `read_only_cache` factory fixture in `tests/conftest.py`, which **hard-links** the read-side subdirs into a throwaway cache — the CTX tile zips and HiRISE JP2s are hundreds of MB, so copying is not viable. The hard link is safe *only because each producer's read and write subdirs are disjoint*; that invariant is documented in the fixture's docstring, because a producer that truncated a linked path would write through to the original inode.
