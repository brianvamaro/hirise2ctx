# Build spec — HiRISE → CTX Rock Abundance pipeline (Weeks 1–2)

> **This is the full original `CLAUDE.md` build spec, preserved verbatim.** It was moved here
> 2026-06-20 to slim the always-loaded `CLAUDE.md`; the invariants/gotchas that must fire every
> session were kept in `CLAUDE.md`, and the detailed reference below lives here. The pipeline it
> describes is **built**; for current phase see [../ROADMAP.md](../ROADMAP.md) and
...
Where it says **VERIFY AT RUNTIME**, do not hardcode — read the value from the data or confirm
against the source, then record what you found.
...
> The live config is `config.yaml` (+ `config_v2.yaml` for the vClaire v2 dataset); read it there.
> The original spec's illustrative block:
...
- Keep the spec authoritative; when reality diverges, update `DECISIONS.md` and note it — don't
  silently drift.
...
CLAUDE.md            # the always-loaded operating manual (invariants + pointers)
```
*(The actual `src/` has grown well beyond this — see the README "Layout" section for the current
tree, including `fm_embeddings.py`, `mapping.py`, `calibration.py`, `modeling/`, `striping.py`, etc.)*
...
## 10. Future work (from the original spec — now LARGELY DONE; see ROADMAP/DECISIONS)
...
dominated by near-zero tiles). *(Done → superseded by the foundation-model recipe; see PLAN_FM.md.)*
...
*(In progress under PLAN_RegionalMap.md validation legs.)*
...
*Originally planned to use CRISM; switched to HiRISE bands 2026-05-30. (Done through Stage 7 Tier 1/2;
parked at the 2026-06-10 usability pivot — see PLAN_Compositional.md / docs/compositional.md.)*
...
## 11. Open items from the original spec (all RESOLVED — recorded in DECISIONS)
- Exact CTX mosaic CRS (read from a downloaded tile) and whether Murray Lab GeoTIFFs support
  `/vsicurl/` range requests. → resolved (Mars_2015 equirectangular clon_0; vsicurl supported).
- The `.dbf` attribute schema of the BoulderNet shapefiles (confidence? size?) → resolved.
- HiRISE `.LBL` fields for backfilling incidence/emission angles → resolved.
