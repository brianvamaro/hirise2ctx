...
> # ✅ THE REBUILD IS DONE (2026-08-25). This table is EMPTY except rows 2–3.
>
> [PLAN_Rebuild.md](../PLAN_Rebuild.md) executed all 12 steps 2026-08-20 → 25 and every §6 number
> is re-derived (DECISIONS 2026-08-25h). **Row 1 (R74) is discharged in full**: the artifacts on
> disk are now what the current code produces, rich prevalence landed on **0.373272** as predicted,
> and the shipped maps (`reports/map_region`, `reports/map_a1`) were re-rendered on the corrected
> global lattice, sha256-verified 156/156 and gated 12/12.
>
> **Rows 2 and 3 stay open, by ruling, not by omission.** Brian ruled the pass **FM-path only**
> (DECISIONS 2026-08-19, decision 4): the frozen recipe is embeddings-only, so Stage 4b *did*
> regenerate `features/` and `context_patches`, but **no GBM sweep and no W1 error atlas were
> re-run**. So for R27 and R28: *features regenerated; downstream tabular numbers not re-derived.*
> Anything in `docs/modeling.md` / `docs/modeling_results.md` that comes off a `lacunarity_shadow_b*`,
> `edge_*` or `nbr_*` column is still on the pre-fix basis and must not be quoted as current.
>
> **Do not use this file as a rebuild plan any more** — it is a record. The next rebuild is v3
> (BoulderNet retrained, more diverse locations), which forces a full rerun regardless.
>
> *Superseded, retained for the record —* **Update (2026-08-18): the isolation gate is CLOSED — the rebuild is unblocked.** All five criteria
