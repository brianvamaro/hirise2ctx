...
| Stage 3 | Provenance binding ✅ **landed 2026-08-06** (input digests + coverage-mask identity + a `shift_id` propagated into Stage 4). Still open: **R65** — confirmed, but its register fix bullet is wrong as specified and the replacement statistic needs a design round; it also requires a Stage-3 re-run, so fold it into the batched rebuild. |
