...
| Stage 2 | R74 tests and provenance ✅ **landed 2026-08-06**; R31 ✅ **fixed 2026-08-06** (overhang now refused, no v2 window affected). Still open: **R66** — resolve before any download/cold-cache path; the cached JP2s are verified intact (46/46 pass a full JPEG2000 tile-part chain walk) so nothing is corrupt today, but a truncated download would be silently zero-filled by GDAL and read as "no HiRISE coverage". **R67** is now **LOW, not medium** — the affected branch is provably unreachable for v2 (39/39 images take `polygon_bbox`) and its register fix bullet is unsafe as written. |
...
| Stage 4 | R29 ✅ **fixed 2026-08-06** (coverage mask now shifts with the polygons; `coreg_mask_shift` provenance). R68 ✅ **CLOSED 2026-08-06** — the pixel-size guard was a tautology (window and tile sidecar come from the same handle; measured 0.0 difference on 49/49); replaced inside `_compute_grid_alignment` by a real origin-phase + negative-origin check, all five mutants killed. R80 is reproduced and covered end to end with projected units, a non-`None` size filter, and a fixture that distinguishes diameter from radius; retained mixed-floor semantics make this path load-bearing. |
