...
| Mapping | R01 ✅ **CLOSED 2026-08-09** (globally anchored grid before inference, both drivers on it, coverage enforced rather than assumed). R14 ✅ **CLOSED 2026-08-10** (atomic staged writes, finite-count verification, sidecar-last commit, content+provenance resume, set-equality assembly, merged manifest). R84 product metadata is emitted as part of R14's sidecar. **Remaining for this gate: R13 only** — record and enforce a context-window nodata threshold; checking only the central 32-px tile admits embeddings whose surrounding 96-px context is mostly nodata. Note R14 made `max_zero_fraction` a resume-match field, so R13's threshold must join it in the `run` block or a post-R13 resume will reuse pre-R13 partials. |
| A1 | R07 ✅ **code fixed 2026-08-09** — one statistic (native, per SeamMap frame, tile support) on both sides, `norm_arm` versioned into `recipe.json` and the recipe hash, and the A1 driver refuses a head that cannot be verified as the A1 arm. Still required before A1 ships: the **re-embed + retrain** under the new arm (folded into the batched rebuild), R08's fallback *contract* (the mechanism is fixed, the choice is not ratified), R38's explicit nodata mask, and A1-specific head/calibration artifacts. Parity must then be re-checked end to end — note the banked η² 0.196→0.141 and the −0.024 AUC came from different A1 definitions and remain non-comparable until then. |
