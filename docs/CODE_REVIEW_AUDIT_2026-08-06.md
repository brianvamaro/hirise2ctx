...
| Mapping | R01 ✅ **CLOSED 2026-08-09** — the globally anchored grid is created before inference and both drivers render onto it; the sweep's coverage is enforced by `uncovered_cells` rather than assumed, products carry `grid_id`, and a resumed run refuses partials from another lattice. Remaining: R13 records/enforces context nodata; R14 prevents false resume; R84 product metadata is emitted. |
