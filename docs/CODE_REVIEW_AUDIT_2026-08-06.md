...
|---|---|
| **R01** | ✅ **CLOSED 2026-08-06 (part 1 `519227c`) + 2026-08-09 (part 2) — rebuild pending.** One globally anchored coarse grid (`COARSE_GRID_ID`, anchor lon0/lat0) is now built *before* inference and threaded through **both** map drivers in a single commit, so A1 cannot land on a different lattice than the baseline. The register was right that a merge-time reprojection is only a stopgap — measured, it caps the error at ≤0.5 cell rather than 0, because the true offsets are fractional (k × 0.125 cell) and `merge` floors them. **Three things the register's fix bullet did not have:** (i) the window sweep silently drops **11 cells per axis** at every non-zero phase, and *each half* of the fix alone still loses cells (1 with only the wider overlap, 10 with only the unpinned final offset) — measured 2×2, and free at 12 offsets/axis either way; (ii) two further merge paths bypass the guard (`striping.mosaic_tiles`, now warning, and `striping_frame_blocks`, now routed through it); (iii) re-anchoring does not change the *definition* of A1's per-frame statistic, so it does not invalidate `models/deployable_a1` the way deriving it from native 5 m DN would — but measured over 74 real frames it does perturb the numbers, by >1 DN on 11 of them and up to 9 DN on the smallest, which makes **R08** (small-frame handling) a precondition for shipping A1 rather than an unrelated finding. Forces the full re-render; see `docs/PENDING_REBUILD.md` row 6 and DECISIONS 2026-08-06x + 2026-08-06y. |
...
| **R07** | ✅ **CODE FIXED 2026-08-09 — re-embed + retrain pending.** This interpretation was right and the original register entry was not: there are **three** defects. (i) *Resolution*, confirmed and quantified over all 39 Stage-2 windows — gain error `IQR_native/IQR_160m` **1.35× median, 2.15× max**, so against the 27.7 training pins by construction the head received a realised IQR of 37.3 median / 59.6 max and ~10× more clipped pixels. (ii) *Unit*, **new**: the training comment claimed "each training window is ~one CTX source frame"; measured, only **10 of 38** windows lie in one frame (22 span two, 3 span three, max four, dominant share median 81 %), so training removed between-*window* scale and deploy between-*frame* scale — the register's resolution-only fix would not have achieved parity. (iii) *Versioning*, **new**: eleven heads share `recipe_hash 86c51a5dca220f63` and none records its arm. Both sides now call one definition (`src.striping.A1_ARM`, native per-frame, tile support, no pixel left at raw DN) and `norm_arm` is stamped into `recipe.json` and the hash. Two blockers found while implementing: `load_frames` required a rendered abundance raster (the 39 windows span 20 tiles, only 26 have one), and the naive implementation cost ~45 min/tile until a bbox pre-filter brought it to 3.4 min. **R01's A1 ordering constraint is dissolved by this fix.** See DECISIONS 2026-08-09a. |
| **R08** | OPEN, but **narrowed and partly pre-empted by R07 (2026-08-09)**. The mixture is gone from the paths that feed the embedder: `a1_normalize_native` normalizes every valid pixel, giving anything in no qualifying frame the tile-wide native statistic, and **raises** rather than returning raw DN if no fallback is supplied. `a1_normalize_per_frame` retains the old raw-DN behaviour and is now documented as a diagnostics-only hazard. First measurement of the affected population: on E-12_N36 the fallback covers **0.0081 %** of valid pixels (1 frame of 81 below the 50-px floor). **CONTRACT RATIFIED 2026-08-10 (Brian) — R08 is CLOSED; no code change was needed, which is the point.** Both halves were settled by measurement, not preference. (1) *Normalize, never drop.* The fallback population is **isolated single pixels** (horizontal run length median 1, p90 2, max 15) at 0.0058–0.0108 % of valid pixels — rasterization-precision gaps inside the dissolved SeamMap polygons, not real coverage holes. Dropping them makes each one nodata, and R13's zero-tolerance context gate then sterilises every cell whose 96-px box touches one: measured cost **3.11 % / 3.37 % / 4.38 % of the tile** (E8_N44 / E4_N44 / E-8_N32) against **0.00 / 0.00 / 0.072 %** under the fallback — a 400–530× amplification in cell-equivalents, i.e. trading 3–4 % of the map to avoid a 1e-4 radiometric approximation. Same scattered-vs-blob asymmetry R13 measured, biting the same way. (2) *The 50-px floor is a tripwire, not a knob*: exactly **1 frame of 214** across four real tiles fell below it (E-12_N36 1/81; E-8_N32 0/54; E4_N44 0/48; E8_N44 0/31), so its value trades nothing off on real data. Both halves are now pinned by tests that fail if the contract is silently "tightened". The separate re-anchoring observation (>1 DN on 11 of 74 small frames) was **dissolved by R07**, which moved the statistic off the baseline's grid entirely. |
| **R13** | **FIXED 2026-08-10 (DECISIONS 2026-08-10b) — CODE FIXED, REBUILD PENDING (`PENDING_REBUILD.md` row 8).** `src.mapping.context_zero_fraction` gates the full 3×32-px box; `predict_window(max_context_zero_fraction=0.0)` with **two** counters, not one; both thresholds recorded in the sweep manifest (so R14's resume cannot mix gate policies), the tile sidecar's `nodata_gate` block and the run record, with de-duplicated masked-cell counts plus a re-tunable histogram. Re-validated on the full real `E-8_N32`: exactly **290** additional cells of 19,685,689, and the own-tile mask reproduces the shipped NaN pattern at 100.000000 %. Threshold 0.0 rests on two legs only — the training set's 0 nodata px in 161,005 context boxes, and the 1.5e-05 cost; the "scattered zeros" leg is withdrawn (DN 0 and DN 1 damage the embedding identically, so this gate cannot see A1's clip speckle). **The A1 arm's gate is DISABLED (1.0) until R38**, and "clip the floor to 1" is ruled out as an R38 remedy in this ordering. `f_region_stageb.py` is knowingly left ungated (F is closed). |
| **R14** | ✅ **CLOSED 2026-08-10.** `write_geotiff` stages a `.tmp` sibling, verifies it and renames (it used to open the destination in `"w"`, which truncates an existing good raster immediately). `verify_geotiff` decodes **blockwise** and checks the **finite count** — the only test that catches the two nodata-shaped kill signatures (a valid 100 %-NaN raster and a half-written one both open, decode, and read their last block cleanly). Tiles commit as a set with the **sidecar last** as the marker, carrying per-raster `bytes`/`sha256`/`shape`/`n_finite` plus a `run` block; resume checks content **and** provenance and names the first failing reason; the assembly gate is set equality, not a count (a count let 719 partials satisfy a 144-window grid); damaged partials are recomputed while foreign ones are refused; the manifest merges instead of clobbering. Two register numbers were wrong and are corrected in DECISIONS 2026-08-10a (719/63.1 %/0.4933, and the `n_windows·(win/S−2)²` invariant is invalidated by R01). Mutation pass 13/13. **No rebuild to repair damage** — all 188 shipped rasters fully decode — but the 26 pre-R14 sidecars are correctly "unverifiable, not reusable". |
...
| **R31** | ✅ **FIXED 2026-08-06.** `extract_ctx_window` now clips once and derives both the read and the transform from the clipped window, and **raises** on any overhang rather than writing one; plus a bounds-readback check in `stage2_one_image`. Measured which directions were actually broken: **west/north are mis-georeferenced** (a 300 px west overhang put the origin exactly 1500 m too far west), **east/south georeference correctly but truncate silently**. **Blast radius none** — cache_v2 39/39 pass, so no shipped v2 label is affected; exactly one v1 window raises (`ESP_057469_2215`, 9,620 m west), already excluded. See DECISIONS 2026-08-06q. |
| **R38** | **FIXED 2026-08-10 (DECISIONS 2026-08-10c) — CODE FIXED, REBUILD PENDING (`PENDING_REBUILD.md` row 9, folds into row 7's A1 re-bake).** Three changes, and the register was right that `[1,255]` alone is not the fix: (1) valid pixels floor at `src.striping.A1_VALID_FLOOR = 1`, so DN 0 means only nodata; (2) `predict_window` takes an **explicit `nodata_mask`** and both A1 drivers pass one derived from the **raw** DN, so coverage is never re-inferred from a transformed array — the durable half, since a floor that merely looks safe stops being safe the next time the transfer function moves; (3) the destroyed texture is counted exactly and separately (`a1_clip_counts_from_hist`, off the histogram `frame_hist_native` already builds — no per-window sum, so no 96-px seam double-count and no resume dependence) and recorded per tile as `a1_clip_*` with `--warn-clip-fraction`. R13 supplied the reason (2) is mandatory: DN 0 and the legal DN 1 damage the frozen embedding **identically to three decimals**, so flooring alone would have made the pixels invisible rather than safe. Brian ruled 2026-08-10: record the loss, do **not** retune `A1_REF` (R07 already cut the damage ~10×). Sibling fix: `a1_stats`/`a1_stats_from_hist` returned a fabricated `iqr = 1.0` on a degenerate frame, which **defeated** `a1_stats_native_tile`'s own `iqr > 0` guard and handed that frame a 27.7× gain; now NaN, so it routes to the fallback as intended. The inline clip copy in `striping_a1_infer_crop.py` now calls `a1_apply`. This is what let R13's A1 context gate flip from disabled to 0.0. |
...
| Stage 2 | R74 tests and provenance ✅ **landed 2026-08-06**; R31 ✅ **fixed**; R66 ✅ **CLOSED 2026-08-06** — `verify_download` + a pure-bytes JPEG2000 walker gate every commit, the two sibling download paths are fixed, and all 46 cached JP2s in both roots walk to `complete`, so nothing is corrupt and no rebuild is forced. R67 ✅ **CLOSED 2026-08-06** at **LOW, not medium** — the branch is provably unreachable for v2 (39/39 take `polygon_bbox`), so no artifact changes. The fallback now prefers the image's own PDS footprint and applies the cos(lat) correction when it cannot. Two register corrections: the cohort spans −63.70 to +52.33 (cos 0.443–0.929), not 11N–46N; and a second unnamed defect — `nominal_hirise_length_m` = 16,000 is exceeded by **13 of 39** footprints (max 43,088 m), and the 6 km width is too narrow for **39 of 39** even after the cos fix. A test was certifying the bug and is rewritten. See DECISIONS 2026-08-06v. **Stage 2 is now clear.** |
| Stage 3 | Provenance binding ✅ **landed 2026-08-06** (input digests + coverage-mask identity + a `shift_id` propagated into Stage 4). R65 ✅ **CLOSED 2026-08-06** — `peak_correlation` is a *conditional* median bounded below by the floor it was screened against (a `>= 0.5` screen rejects **0 of 38**), and on the block path it does not score the shift actually applied. Its register fix bullet was refuted before implementation — an unconditional median reads ~0.75 on a perfectly registered image with 25 % junk blocks — so Stage 3 now emits the *components* (`all_block_peak`, `confident_fraction`, `*_is_conditional` labels, `quality_version`) rather than a conflated composite. Provenance only; **Stage 3 is now clear**. Requires the Stage-3 re-run already in the DAG. See DECISIONS 2026-08-06w. |
| Stage 4 | R29 ✅ **fixed 2026-08-06** (coverage mask now shifts with the polygons; `coreg_mask_shift` provenance). R68 ✅ **CLOSED 2026-08-06** — the pixel-size guard was a tautology (window and tile sidecar come from the same handle; measured 0.0 difference on 49/49); replaced inside `_compute_grid_alignment` by a real origin-phase + negative-origin check, all five mutants killed. R80 ✅ **CLOSED 2026-08-06** — reproduced and covered end to end in projected units with a non-`None` size filter and a fixture that separates diameter from radius *and from four other size metrics*; a concave shape now also pins polygon area against bbox/hull area, which had been unpinned (on `ESP_046328_2180`, bbox area would silently retain 3,466 of 6,360 real drops). The realised **physical** floor is emitted per image as `realised_size_basis.realised_physical_min_size_m`. See DECISIONS 2026-08-06s + 2026-08-06u. |
...
| Mapping | R01 ✅ **CLOSED 2026-08-09** (globally anchored grid before inference, both drivers on it, coverage enforced rather than assumed). R14 ✅ **CLOSED 2026-08-10** (atomic staged writes, finite-count verification, sidecar-last commit, content+provenance resume, set-equality assembly, merged manifest). R84 product metadata is emitted as part of R14's sidecar. R13 ✅ **CLOSED 2026-08-10** — the full 3×32-px context box is gated (`max_context_zero_fraction`, default 0.0 on the baseline arm, **1.0/disabled on A1 until R38**), attributed to its own counter, and *recorded*: both thresholds sit in the sweep manifest — so the R14 coupling is closed and a post-R13 resume cannot reuse pre-R13 partials — as well as in each tile sidecar's `nodata_gate` block and the run record, with de-duplicated masked-cell counts and a re-tunable histogram. Re-validated on the full real `E-8_N32`: exactly **290** additional cells of 19,685,689 (own-tile mask reproduces the shipped NaN pattern at 100.000000 %). Two adjacent defects fixed in passing: `predict_window`'s own-tile default was 0.5 while every driver passed 0.3, and `scripts/parity_check.py` raised `TypeError` on every invocation (7 parameters, 8 positional arguments). **✅ THE MAPPING GATE IS NOW CLOSED** — nothing here blocks the corrected baseline map except the rebuild's own safety gate (isolation criterion 5). |
| A1 | R07 ✅ **code fixed 2026-08-09** — one statistic (native, per SeamMap frame, tile support) on both sides, `norm_arm` versioned into `recipe.json` and the recipe hash, and the A1 driver refuses a head that cannot be verified as the A1 arm. R38 ✅ **CLOSED 2026-08-10** — clip floor off the nodata sentinel, an explicit nodata mask threaded from the raw DN into both gates, and the destroyed texture counted separately per tile; this is also what let R13's context gate turn on for this arm. R08 ✅ **CLOSED 2026-08-10** — contract ratified on measurement (normalize the fallback population, never drop it: dropping costs 3.1–4.4 % of a tile to avoid a 1e-4 approximation; the 50-px floor is a tripwire, 1 frame in 214). No code change; the tests are the ratification. Still required before A1 ships: the **re-embed + retrain** under the new arm (folded into the batched rebuild — R38 changes the training input too, by design), A1-specific head/calibration artifacts, The **A1 resume-match guard** — found 2026-08-10 while landing R13 — is ✅ **CLOSED 2026-08-11**: `striping_a1_map.py` deleted a `_sweep.json` it never wrote and never called `sweep_manifest`, so R14's protection covered `map_region.py` only. It now builds an `a1_sweep_manifest` (map_region's fields **plus** `norm_arm`, `a1_ref`, `a1_clip_floor`, `a1_min_frame_px` and an `a1_seammap_digest` that hashes every shapefile sibling, the `.prj` included — the frames are reprojected before rasterization, so a changed projection moves every per-frame statistic without touching a coordinate), writes and checks it, passes it to `tile_is_reusable` instead of `None`, and records it as the sidecar's `run` block. It digests A1's *inputs* rather than the derived statistics so the whole identity is cheap enough to build **before** the ~3 min streaming pass — a committed tile is now skipped without paying for a read it does not need. Parity must then be re-checked end to end — note the banked η² 0.196→0.141 and the −0.024 AUC came from different A1 definitions and remain non-comparable until then. |
