...
Union = **54 tiles**. Round 2 (planned, 69 more) takes it to 123. Any analysis that naively pools the
...
lat 20→48, i.e. 11 × 7 = 77 tile slots for 54 actual tiles. **~30% of the bbox is nodata** and every
...
scale over all 54 tiles. Deliverable: **how much apparent abundance variation is attributable to CTX
...
spans > 180° and its linear mean is then wrong. Over 54 tiles this **will** trigger — use circular
...
- **§4** Per-frame η² over the union (extending the 26-tile 0.1444 to all 54), and the frame-boundary
...
dx=dy=0); §2 leg 1 re-run over the full union — does the weak +0.05 hold on 54 tiles; §3 **leg 2, the
...
|---|---|---|---|
| 1 | `scripts/map_union.py` + `src/map_validation.py` + tests | — | **everything else needs this** |
...
| 5 | Notebook 32 — illumination | 1 | 54 SeamMap fetches over vsicurl |
