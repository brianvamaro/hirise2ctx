...
| 8 | Train the all-data **deployable head**, per arm | ✅ **DONE 2026-08-23, ~100 s/arm** → `models/deployable_g2/a5ffca2dcc536855` + `models/deployable_a1_g2/7bbd8a8e1d377f6e` | `python -u scripts/train_deployable_head.py --dataset-dir dataset_v2 --store-name fang_embeddings --target fa_gt_1e-2 --norm-arm none --out models/deployable_g2` (and `--store-name fang_embeddings_a1 --norm-arm a1 --out models/deployable_a1_g2`) | R07: `norm_arm` is **part of the recipe hash** — the two arms must produce *different* hashes. Eleven heads previously shared `86c51a5dca220f63` with the arm recorded nowhere; if the new baseline and A1 hashes match, the fix is not in effect. |
