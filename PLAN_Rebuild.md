...
⚠ **They are floors, not forecasts.** Step 1 measured **~9 min against the ~2.6 min** its mtimes implied, because R23 added a byte-integrity scan of every source `.shp` plus a score-rank analysis of the null-geometry population — work the original run never did. Stage 2 (R74 mask + digests), Stage 3 (R65 components + `shift_id`) and Stage 4 (R29/R80) all gained provenance work in the fixing tranche too, so expect the same under-read. The 1.5–2 day total still holds.
...
| 3 | Stage 3 — co-registration + QA | local **(unknown)** | `python -u scripts/run_stage3.py --config config_v2.yaml --all` | **CRS sanity (moved here from step 1, 2026-08-20): residual HiRISE↔CTX offset must be O(200 m), NOT km — if km, ABORT, the CRS handling is wrong.** Binds to the exact Stage-2 digests; emits `shift_id` and the R65 *components* (`all_block_peak`, `confident_fraction`, `*_is_conditional`, `quality_version`) rather than a conflated `peak_correlation`. Sanity: centroid offsets under `sanity.centroid_max_km: 15.0`. |
