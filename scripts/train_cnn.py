...
from src.modeling.evaluate import per_fold_metrics, aggregate_fold_metrics
...
def _split_inner_val(fold, target_col: str):
    """Same rotation rule as src.modeling.evaluate.run_loio."""
    ...
    y = fold.y_train[target_col].to_numpy(dtype=np.float64)
    return (
        fold.keys_train[inner_train_mask].reset_index(drop=True), y[inner_train_mask],
        fold.keys_train[inner_val_mask].reset_index(drop=True), y[inner_val_mask],
    )


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--patch-size-px", type=int, choices=[32, 64], required=True)
    ap.add_argument("--scale-idx", type=int, required=True, choices=[0, 1, 2, 3])
    ...
    snapshot = {
        "variant": f"cnn_log1p_huber_S{args.patch_size_px}",
        "target_col": args.target_col,
        "scheme": args.scheme,
        "scale_idx": args.scale_idx,
        "tile_size_px": tile_size,
        "model": {"variant": f"cnn_log1p_huber_S{args.patch_size_px}", "params": asdict(params)},
    }
    cfg_hash = _config_hash(snapshot)
    snapshot["config_hash"] = cfg_hash

    out_dir = MODELS_ROOT / snapshot["variant"] / cfg_hash / f"scale_S{tile_size}_P{args.patch_size_px}"
    ...
    for fold in iter_loio_folds(args.scheme, scale_idx=args.scale_idx):
        ...
        keys_tr, y_tr, keys_vl, y_vl = _split_inner_val(fold, args.target_col)
        ...
        print(f"  {tag}  rmse_log1p={m['rmse_log1p']:.4g}  auc={m['presence_auc']:.3f}")

    aggregate = aggregate_fold_metrics(per_fold_records)
    ...
    print(f"\nDONE: {snapshot['variant']} scale_idx={args.scale_idx}")
