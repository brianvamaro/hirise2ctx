...
| [PLAN_FBuild.md](PLAN_FBuild.md) | The 907-frame regional F build: ISIS per-frame processing → H1-centered per-frame inference → H4 leveling on the 3,584-edge graph (pre-declared trend-guard method) → composite + H6 provenance layers + acceptance gates | **APPROVED — EXECUTING (2026-07-23)** — reopening call = reopen-with-guards (Brian); §0 P1–P5 all cleared; H1/H4 code audit 2026-07-23 green; comparison vs {mosaic, A1} on quality+cost mandated (§5.1). V1/V2/V5 done. **Stage A DONE** (907 cubes → GeoTIFFs); **Stage B DONE** (906/907 per-frame logit npzs, on Sherlock scratch); **Stage C BUILT 2026-07-28** (`src/leveling.py` + `scripts/f_region_stagec.py`, 32 tests, guard-1 binding checked on real MOLA/THEMIS); **Stage D + 6 gates + §5.1 BUILT 2026-07-29** (`src/fcompose.py`, `src/fgates.py`, `f_region_staged.py`, `f_region_gates.py`, `f_map_compare.py`, `bank_calibration_f.py`, `striping_a1_map.py`; 63 tests; commit `afe6fce`). Five Brian rulings 2026-07-28 are baked in — most importantly **gate 1 is now windowed + floor-relative**, because at 907-frame scale the literal η²≤0.05 sits below the geological floor (mosaic block η² 0.3575 vs its own null 0.2837) while the detrended reading is already passed by the un-mitigated map; the **mosaic baseline is banked** (median-window η² 0.1222 vs null p95 0.0676, ratio 1.65). Also RUN: the F-path CalibrationLayer re-bank (Tier-1 LOIO ECE 0.0280 PASS, pooled Tier-2 top_ratio 0.8783 PASS). **Next = RUN it**: transfer the npzs (~2 GB) then Stage C → D → gates → §5.1 (~30 min laptop; SHERLOCK_RUN Part J), plus 2 GPU steps for the A1 comparison row → **the F-build verdict** |
