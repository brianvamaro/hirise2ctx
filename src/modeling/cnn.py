...
class SmallCNNRegressor:
    ...
    def fit(
        ...
        *,
        ...
        train_patches, train_rows = gather_patches(self._train_keys, self.params.patch_size_px)
        ...
        if self._val_keys is not None and self._val_y is not None and len(self._val_keys) > 0:
            val_patches, val_rows = gather_patches(self._val_keys, self.params.patch_size_px)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        # baseline (0.0).
        n_total = X.shape[0]
        patches, valid_rows = gather_patches(keys, self.params.patch_size_px)
